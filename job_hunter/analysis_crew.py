...
class JobAnalysisCrew:
    ...
    def run_analysis(self):
        ...
        agent_humanizer = Agent(
            role='Senior Executive Editor & Stealth Writer',
            goal='Refine the cover letter to be 100% professional and indistinguishable from human-written text by advanced AI detectors. Eliminate all robotic patterns, standard templates, and over-conversational "AI-humanizing" fillers.',
            backstory="You are an expert editor and career coach who understands the internal mechanics of AI detectors like QuillBot and GPTZero. You know that 'humanizing' isn't about being casual; it's about breaking predictable structural patterns while maintaining a high-level, sophisticated, and authoritative professional tone. You specialize in 'Burstiness' and 'Perplexity', ensuring the text has a natural, non-linear flow that mimics high-level human thinking, especially for the disciplined German corporate market.",
            verbose=True,
            allow_delegation=False,
            llm=llm
        )
...
Example JSON: {{"cover_letter": "Sheikh Ali Mateen\\nBerlin, Germany\\n..."}}""",
        ...
        task_humanizer = Task(
            description=f"""Perform a deep structural and linguistic refinement of the cover letter to bypass advanced AI detection (QuillBot, GPTZero) while maintaining a strict 100% formal and professional tone.

STRICT HUMANIZATION GUIDELINES:
1. BREAK THE TEMPLATE: Do NOT start with "I am writing to express my interest" or "I was excited to see". Instead, start with a direct observation about the company's challenge, a specific value proposition, or a strong hook related to the role's impact.
2. AVOID LLM TRANSITIONS: Completely ban transition words like "Furthermore,", "Moreover,", "Additionally,", "In conclusion,", "Consequently," and "Notably,". Use direct, logical connections between ideas as a human would.
3. VARY STRUCTURE (BURSTINESS): Mix short, punchy statements with more complex, nuanced sentences. Avoid the standard 3-sentence paragraph structure.
4. PERPLEXITY & VOCABULARY: Use precise, high-level professional vocabulary but avoid "over-optimized" SEO-style keywords that sound like they were generated by an algorithm.
5. NO CONVERSATIONAL FILLERS: As requested, avoid "Honestly,", "Pretty neat,", "I've got,", "Diving deep,". Professional humanization means authentic voice, not casualness.
6. FORBIDDEN OPENINGS: Never use "I am writing to..." or "I am thrilled to...". Start with the 'Why' or the 'Impact'.

FINAL ANSWER MUST BE JSON with keys 'humanized_cover_letter' and 'humanization_score'.
The humanization_score (0-100) must be a strict self-assessment of how well the text masks its AI origins while remaining elite-level professional.
Example JSON: {{"humanized_cover_letter": "The current transformation of the logistics sector requires...", "humanization_score": 99}}""",
