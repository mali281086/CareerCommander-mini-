...
def render_home_view(db):
            ...
            with st.spinner("AI is analyzing your resumes..."):
                ...
                for name, data in st.session_state['resumes'].items():
                    ...
                    if suggestions:
                        ...
                        st.session_state['resumes'][name]['target_keywords'] = kw_str
                        # Force widget to refresh by removing its state key
                        if f"kw_{name}" in st.session_state:
                            del st.session_state[f"kw_{name}"]
                        
                        # Save to history so it shows up in "Recent"
                        db.save_resume_title_history(name, suggestions)
                        success_count += 1
                    else:
                        fail_count += 1
    ...
    # Hardcoded values since we removed the UI toggles
    use_browser_analysis = True
    deep_scrape_toggle = True
    ...
    with m_col2:
        available_platforms = ["LinkedIn", "Indeed", "Xing", "Stepstone", "ZipRecruiter"]
